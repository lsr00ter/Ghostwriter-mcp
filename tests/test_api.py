...
class TestWriteObjects(ApiTestCase):
    ...
    async def test_create_client_nests_extra_fields(self):
        ...
        obj = self.last_body()["variables"]["object"]
        self.assertEqual(obj["extra_fields"], {"custom": 1})
    ...
    async def test_create_finding_uses_schema_field_names(self):
        ...
        obj = self.last_body()["variables"]["object"]
        self.assertEqual(obj["cvss_score"], 7.5)
        self.assertEqual(obj["cvss_vector"], "CVSS:3.1/AV:N")
        self.assertNotIn("cvssScore", obj)
        self.assertNotIn("cvssVector", obj)
...
class TestSchemaConformance(ApiTestCase):
    ...
    ``tests/schema_fixtures.json`` is derived from Ghostwriter's
    ``DOCS/schema.graphql``. Update it when targeting a newer release.
    """
    ...
    async def test_create_client_payload(self):
        ...
        await gw.create_client(
            "Acme", "AC", "ACME", address="a", note="n", extra_fields={"x": 1}
        )
